...
" pydoc (the buffer will be called '__doc__.py', Pythonic, isn't it ;-) ). The
...
function! s:ShowPyDoc(name, type)
    ...
    if bufloaded("__doc__.py")
        let l:buf_is_new = 0
        if bufname("%") == "__doc__.py"
            " The current buffer is __doc__.py, thus do not
            " recreate nor resize it
            let l:pydoc_wh = -1
        else
            " If the __doc__.py buffer is open, jump to it
            if exists("g:pydoc_use_drop")
                execute "drop" "__doc__.py"
            else
                execute "sbuffer" bufnr("__doc__.py")
            endif
            let l:pydoc_wh = -1
        endif
    else
        let l:buf_is_new = 1
        execute g:pydoc_open_cmd '__doc__.py'
